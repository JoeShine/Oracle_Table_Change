...
if __name__ == "__main__":
    ...
    # ================================================================
    # 构建后处理：生成 ZIP 包 + CHM 安全修复说明
    # ZIP 格式不保留 NTFS Zone.Identifier ADS，解压后自动解除锁定
    # ================================================================
    import zipfile
    from datetime import datetime

    chm_path = Path(output)
    if chm_path.exists():
        # 1. 生成 ZIP 包
        zip_name = chm_path.stem + ".zip"
        zip_path = chm_path.parent / zip_name
        print(f"\n生成 ZIP 安全包: {zip_path}")
        with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as zf:
            zf.write(chm_path, chm_path.name)

        zip_size = zip_path.stat().st_size
        print(f"  ZIP 大小: {zip_size:,} bytes ({zip_size / 1024:.1f} KB)")

        # 2. 生成 CHM 安全说明文件
        readme_content = f"""# CHM 帮助手册 - Windows 安全警告说明
...
========================================
  ...
  ZIP 安全包:   {zip_path}
  安全说明:     {readme_path}
  修复脚本:     scripts/chm_unblock.ps1
                scripts/chm_unblock.bat

  Windows 用户请优先使用 ZIP 包分发，
  解压后 CHM 自动解除安全锁定。
========================================
""")